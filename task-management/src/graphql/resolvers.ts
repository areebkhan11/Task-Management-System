...
const resolvers = {
  Query: {
    ...
    tasks: async (_: unknown, __: unknown, context: any): Promise<ITask[]> => {
      ...
      return await Task.find({ assignedTo: user.userId }).populate("assignedTo");
    },
  },

  Mutation: {
    register: async (_: unknown, { name, email, password, role }: RegisterArgs): Promise<IUser> => {
    ...
    createTask: async (_: unknown, { title, description, assignedTo }: { title: string; description: string; assignedTo?: string }, context: any): Promise<ITask> => {
      const user = authenticate(context.token);
      if (user.role !== "admin") throw new AuthenticationError("Only admins can add tasks");
  ...
  Subscription: {
    taskUpdated: {
        subscribe: (): AsyncIterator<any> => pubsub.asyncIterableIterator('TASK_UPDATED'),
    },
  },
};
